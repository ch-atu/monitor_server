...
class WindowsStat(models.Model):
    ...
    cpu_type = models.CharField("CPU型号", max_length=64, blank=True, null=True)
    ...
    physical_mem_total = models.FloatField("物理内存总大小(B)", blank=True, null=True)
    physical_mem_free = models.FloatField('空闲物理内存(B)', blank=True, null=True)
    physical_mem_used = models.FloatField('已使用物理内存(B)', blank=True, null=True)
    physical_mem_used_rate = models.FloatField("物理内存使用率", blank=True, null=True)
    virtual_mem_total = models.FloatField('虚拟内存总容量(KB)', blank=True, null=True)
    virtual_mem_free = models.FloatField('虚拟内存空闲(KB)', blank=True, null=True)
    virtual_mem_used = models.FloatField('已使用的虚拟内存', blank=True, null=True)
